...
int MP4_BoxOpen( stream_t * p_stream)
{
    return stream_Open(p_stream);
}

int MP4_BoxClose( stream_t *p_stream)
{
    return stream_Close(p_stream);
}
...
int MP4_ReadBoxCommon( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_box->i_size )
    {
        if MP4_BOX_TYPE_ASCII()
            printf( "found Box: %4.4s size %"PRId64" %"PRId64"\n",
...
#ifdef MP4_VERBOSE
    if MP4_BOX_TYPE_ASCII()
        printf( "skip box: \"%4.4s\"\n", (char*)&p_box->i_type );
    else
        printf( "skip box: \"c%3.3s\"\n", (char*)&p_box->i_type+1 );
#endif
    return 1;
}
...
#ifdef MP4_VERBOSE
    printf( "read box: \"tfxd\" version %d, flags 0x%x, "\
...
#ifdef MP4_VERBOSE
    printf( "read box: \"tfrf\" version %d, flags 0x%x, "\
            "fragment count %"PRIu8"\n", p_tfrf_data->i_version,
                p_tfrf_data->i_flags, p_tfrf_data->i_fragment_count );

    for( uint8_t i = 0; i < p_tfrf_data->i_fragment_count; i++ )
    {
        TfrfBoxDataFields_t *TfrfBoxDataField = &p_tfrf_data->p_tfrf_data_fields[i];
        printf( "\"tfrf\" fragment duration %"PRIu64", "\
...
#ifdef MP4_VERBOSE
    printf( "es_cat is %"PRIu8", birate is %"PRIu32"\n",
...
#ifdef MP4_VERBOSE
    printf( "read box: \"sidx\" version %d, flags 0x%x, "\
...
static int MP4_ReadBox_tfhd(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    {
        printf("'tfhd' box with version != 0. "\
    ...
    {
        printf( "'duration-is-empty' flag is present "\
...
static int MP4_ReadBox_trun(  stream_t *p_stream, MP4_Box_t *p_box )
{
...
#ifdef MP4_VERBOSE
    printf( "read box: \"trun\" version %u flags 0x%x sample count %u\n",
    ...
    {
        MP4_descriptor_trun_sample_t *p_sample = &p_box->data.p_trun->p_samples[i];
        printf( "read box: \"trun\" sample %4.4u flags 0x%x "\
...
static int MP4_ReadBox_mdhd( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    printf( "read box: \"mdhd\" creation %s modification %s time scale %d duration %s language %3.3s\n",
                  s_creation_time,
                  s_modification_time,
...
static int MP4_ReadBox_dvc1( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    {
        printf("unsupported VC-1 profile (%"PRIu8"), please report", i_profile );
        MP4_READBOX_EXIT( 0 );
    }
...
#ifdef MP4_VERBOSE
    printf(
             "read box: \"enda\" little_endian=%d\n", p_enda->i_little_endian );
#endif
    MP4_READBOX_EXIT( 1 );
}
...
#ifdef MP4_VERBOSE
    printf( "read box: \"trkn\" number=%i\n", p_trkn->i_track_number );
#endif
    if( i_data_len > 15 )
    {
       MP4_GET2BYTES( p_trkn->i_track_total );
#ifdef MP4_VERBOSE
       printf( "read box: \"trkn\" total=%i\n", p_trkn->i_track_total );
#endif
    }
...
static int MP4_ReadBox_sample_soun( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    else if( p_box->data.p_sample_soun->i_qt_version == 2 && i_read >= 36 )
    {
        ...
        {
            printf( "invalid stsd V2 box defaults\n" );
            MP4_READBOX_EXIT( 0 );
        }
        /* !Checks */
...
#ifdef MP4_VERBOSE
        printf( "read box: \"soun\" V2 rate=%f bitsperchannel=%u "
    ...
    {
        printf( "DRM protected streams are not supported.\n" );
        MP4_READBOX_EXIT( 0 );
    }
...
#ifdef MP4_VERBOSE
    printf( "read box: \"cprt\" language %3.3s notice %s\n",
...
#ifdef MP4_VERBOSE
    printf( "i_sample_count is %"PRIu32"", i_sample_count );
    if ( i_sample_count > 3 )
        printf(
...
static int MP4_ReadBox_tfra( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    printf( "number_of_entries is %"PRIu32"\n", i_number_of_entries );
    printf( "track ID is: %"PRIu32"\n", p_tfra->i_track_ID );
#endif
...
static int MP4_ReadBox_default( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_box->p_father->i_type == ATOM_stsd )
    {
        ...
        switch( p_hdlr->data.p_hdlr->i_handler_type )
        {
            ...
            default:
                printf("unknown handler type in stsd (incompletely loaded)\n" );
                return 1;
        }
    }
                                    ...
                                    MP4_Box_t *p_box, unsigned int i_level )
{
    ...
    {
        if MP4_BOX_TYPE_ASCII()
            printf( "dumping root Box \"%4.4s\"\n",
                              (char*)&p_box->i_type );
        else
            printf( "dumping root Box \"c%3.3s\"\n",
                              (char*)&p_box->i_type+1 );
    }
    else
    {
